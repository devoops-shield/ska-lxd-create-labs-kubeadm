...
lxc launch ubuntu:20.04 k8s-1 --vm --profile default --profile vm
...
echo "Creating ignite worker nodes..."
sleep 90
export MASTER_IP=$(lxc exec k8s-1 -- sudo  --user ubuntu --login sudo -s hostname -I | awk '{print $1}')
echo "export MASTER_IP=$MASTER_IP" > master_ip.sh
cat common.sh | lxc exec k8s-1 -- sudo  --user ubuntu --login sudo -s
cat master.sh | lxc exec k8s-1 -- sudo  --user ubuntu --login sudo -s
